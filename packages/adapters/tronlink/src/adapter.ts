...
export function supportTronLink() {
    return !!((window.tron && window.tron.isTronLink) || window.tronLink || window.tronWeb);
}
/**
...
export class TronLinkAdapter extends Adapter {
    ...
    async connect(): Promise<void> {
        try {
            ...
            await this._checkAndUpdateWallet();
    ...
    private _updateWallet = () => {
        if (window.tron && window.tron.isTronLink) {
            this._supportNewTronProtocol = true;
            this._supportTronLink = true;
            this._wallet = window.tron;
            this._listenTronEvent();
            const address = (this._wallet.tronWeb && this._wallet.tronWeb.defaultAddress?.base58) || '';
            if (address) {
                this.setAddress(address);
                this.setState(AdapterState.Connected);
            } else {
            ...
            if (window.tronWeb.ready) {
                this.setAddress(this._wallet.tronWeb.defaultAddress?.base58 || '');
                this.setState(AdapterState.Connected);
            } else {
    ...
    private async _checkAndUpdateWallet() {
    ...
    private setState(state: AdapterState) {
        ...
        if (state !== this.state) {
            ...
            } else if (state === AdapterState.Disconnect && old === AdapterState.Connected) {
